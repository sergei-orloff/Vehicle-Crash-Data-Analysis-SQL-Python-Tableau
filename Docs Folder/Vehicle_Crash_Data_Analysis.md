...
Now that we have the raw data, we should follow a structured approach to explore and clean it. The dataset is named **"crash_conditions"** and contains information about vehicle crashes.
...
```SQL
-- 1.2 Preview the First Few Rows
SELECT * FROM crash_conditions LIMIT 10;
```
...
### 1.1 Check the Structure of the Table: ![Check the Structure of the Table](FIGURES/DESCRIBE_crash_conditions.png)
...
-- CHECK if replaced
SELECT REPORT_NUMBER, ROAD_SURFACE_CONDITION_DESC FROM crash_conditions
...
SELECT REPORT_DATE FROM crash_conditions
...
This query uses a `CASE` statement to map the state abbreviations from the `crash_conditions` table to the full state names used in the `population_by_state_oct24` table. I've also added table aliases (`c` and `p`) to make the query more readable.
